<template>
  <div v-if="item?.post_payment_info_required" class="mt-3 rounded-xl border border-amber-200 bg-amber-50/60 p-4 text-sm dark:border-amber-900/70 dark:bg-amber-950/20">
    ...
    <div v-if="savedInfo && !editing" class="mt-3 space-y-1.5 text-xs">
      <div><span class="text-muted-foreground">{{ t('orderDetail.accountEmailLabel') }}：</span>{{ savedInfo.account_email }}</div>
    ...
    <form v-else class="mt-3 grid gap-3" @submit.prevent="submit">
      <label class="grid gap-1.5">
        <span class="text-xs font-medium text-foreground">{{ t('orderDetail.accountEmailLabel') }}</span>
        <input v-model.trim="accountEmail" type="email" autocomplete="email" required maxlength="254"
          class="h-10 rounded-lg border bg-background px-3 text-sm outline-none focus:border-primary"
          :placeholder="t('orderDetail.accountEmailPlaceholder')" />
      </label>
      <label class="grid gap-1.5">
...
const accountEmail = ref(savedInfo.value?.account_email || '')
...
const cancelEdit = () => {
  accountEmail.value = savedInfo.value?.account_email || ''
